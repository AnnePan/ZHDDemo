...
#import "PAMBProgressHUD.h"

@interface PAMBProgressHUD ()
...
@implementation PAMBProgressHUD
...
//初始化
+(PAMBProgressHUD*)shareWithInitWindow:(UIWindow *)window
{
    return [[PAMBProgressHUD alloc]initWithView:window];
}
...
- (void)updateProgress {
    [(PAMBRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == MBProgressHUDModeDeterminate) {
#if __has_feature(objc_arc)
        self.indicator = [[PAMBRoundProgressView alloc] init];
#else
        self.indicator = [[[PAMBRoundProgressView alloc] init] autorelease];
#endif
}
...
+ (PAMBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	PAMBProgressHUD *hud = [[PAMBProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
#if __has_feature(objc_arc)
	return hud;
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[PAMBProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		PAMBProgressHUD *HUD = (PAMBProgressHUD *)viewToRemove;
...
- (void)layoutSubviews {
    ...
    if (nil != self.labelText) {
        ...
        CGSize dims = [self.labelText sizeWithFont:self.labelFont constrainedToSize:CGSizeMake(240, 560) lineBreakMode:NSLineBreakByCharWrapping];
...
@implementation PAMBRoundProgressView
